...
const TableOfContents: React.FC<TableOfContentsProps> = ({ items }) => {
  ...
  return (
    <div className="sticky top-20 animate-fade-in-right ml-6 py-4">
        ...
        {items.map((item) => (
          <li key={item.id}>
            <a 
              href={`#${item.id}`}
              className={`
                block py-1 duration-250
                ${activeSection === item.id 
                  ? 'text-devscribe-teal font-semibold scale-105' 
                  : 'font-semibold text-devscribe-text-secondary'}
                hover:text-white hover:translate-x-1 hover:scale-105
              `}
            >
              {item.title}
            </a>
